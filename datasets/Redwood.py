...
class RedwoodDataset(data.Dataset):
    ...
    def get_data(self, index):
        sorted_keys = sorted(self.gt_trans.keys(),
                             key=lambda x: (int(x.split('@')[1].split('_')[0]), int(x.split('@')[1].split('_')[1])))
        key = sorted_keys[index]
        ...
        # src_pcd.transform(np.linalg.inv(src_pose)) # move pcd to local coord, now need (src_pose) to world
        # tgt_pcd = make_point_cloud(tgt_keypts)
        # tgt_pcd.transform(np.linalg.inv(tgt_pose)) # move pcd to local coord, now need (tgt_pose) to wor ld
        # src_keypts = np.array(src_pcd.points)
        # tgt_keypts = np.array(tgt_pcd.points)
        ...
        src_features = src_features[src_sel_ind, :]
        tgt_features = tgt_features[tgt_sel_ind, :]
        src_keypts = src_keypts[src_sel_ind, :]
        tgt_keypts = tgt_keypts[tgt_sel_ind, :]
        ...
        return torch.from_numpy(src_keypts.astype(np.float32)).cuda()[None], \
            ...
            torch.from_numpy(gt_trans.astype(np.float32)).cuda()[None], key_tuple
